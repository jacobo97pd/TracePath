...
def _mix_for_world(world_index: int) -> Dict[str, int]:
    if world_index <= 2:
        return {"easy": 16, "medium": 6, "hard": 2}
    if world_index <= 5:
        return {"easy": 12, "medium": 8, "hard": 4}
    if world_index <= 8:
        return {"easy": 9, "medium": 9, "hard": 6}
    return {"easy": 7, "medium": 8, "hard": 9}
...
def _build_worlds(levels: Sequence[Dict[str, Any]]) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    for level in levels:
        pools[_bucket(str(level.get("difficultyTag", "d5")))].append(level)
    ...
    while any(pools.values()):
        ...
        if len(chunk) < WORLD_SIZE:
            for bucket_name in ("medium", "hard", "easy"):
        ...
        for level in chunk:
            ...
            difficulty_mix[_bucket(str(level.get("difficultyTag", "d5")))] += 1
        worlds.append(
            {
                "world": world_index,
...
def main() -> None:
    ...
    randomized_order, worlds = _build_worlds(rebalanced)
